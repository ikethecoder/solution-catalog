...
canzea --config_git_commit --template=roles/configdb/consul-1.x/config/etc/systemd/system/multi-user.target.wants/consul.service /etc/systemd/system/multi-user.target.wants/consul.service


#// Prevent rogue nodes joining the cluster

canzea --config_git_commit --template=roles/configdb/consul-1.x/config/config.json /etc/consul.d/config.json
