resource "canzea_resource" "cicd-pipeline-deploy" {
    ...
    attributes = {
        filename = "ecosystems/${var.es_id}/workspaces/${var.deploy_workspace}/pipeline-${var.project}.gocd.yaml"
        definition = <<-EOT

            format_version: 3
            pipelines:
                ${var.tenant_id}-${var.project}-${var.deploy_workspace}:
                    group: ${var.tenant_id}
                    ...
                    - vault:
                        ...
                        - script: |
                            ...
                            vault read -field data -format yaml secret/tenants/${var.tenant_id}/services/${var.project} > env.yaml
                    ...
                    - deploy:
                        ...
                        tasks:
                        - fetch:
                            pipeline: ${var.tenant_id}-${var.project}-${var.deploy_workspace}
                            stage: vault
                            job: vault
                            source: artifacts
                            destination: .
                    ...
                    - deploy_static:
                        ...
                        - script: |
                            ...
                            BUCKET_PATH="${var.dns_prefix}.${var.deploy_workspace}.ws"
