...
class JSNpmBuild
    def createDeployPipeline (parameters)
        ...
        task = JSON.parse(t.process taskTemplateDockerCli, {"workdir" => "es-catalog/ecosystems/#{ENV['ECOSYSTEM']}/components/#{attributes['project']}", "arguments" => ["create", "--name", "#{attributes['project']}-deploy", "-p", "#{attributes['port']}:#{attributes['internal-port']}", "#{attributes['project']}-deploy"] })
        job['tasks'].push (task)
    ...
    def createPipeline (parameters)
        ...
        task = JSON.parse(t.process taskTemplate2, {"project" => attributes['project'], "arguments" => ["#{project}-task", "npm", "install"] })
        job['tasks'].push (task)

        task = JSON.parse(t.process taskTemplate2, {"project" => attributes['project'], "arguments" => ["#{project}-task", "npm", "run", "build"] })
        job['tasks'].push (task)
    ...
    def preparePipelineScripts(parameters)
        ...
        deployDockerFile = t.process("#{ENV['CATALOG_LOCATION']}/helpers/config_writers/pipelines/commands-deploy/node.script", parameters)
