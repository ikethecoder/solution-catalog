...
class MkdocsBuild
    ...
    def createPipeline (parameters)
        ...
        task = JSON.parse(t.process taskTemplate2, {"project" => attributes['project'], "arguments" => ["-e", "GO_PIPELINE_LABEL", "canzea/canzea_cli", "template", "#{attributes['project']}", "#{attributes['project']}"] })
        job['tasks'].push (task)
