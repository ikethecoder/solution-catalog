...
(cd ${CATALOG_LOCATION}/blocks/flows-gateway/docker && docker build --tag local_gw .)

docker create --name flows-gateway -p 8000:8000 -e adminRoot=/gwadmin/ -e ECOSYSTEM -e ECOSYSTEM_LABEL="Ecosystem SSO Gateway" -e ADMIN_OAUTH2_CALLBACK=${SERVICE_FLOWS_GATEWAY_ESADMIN_CREDENTIALS_URL}/auth/strategy/callback -e ADMIN_OAUTH2_CLIENT_ID=flows-gateway -e ADMIN_OAUTH2_CLIENT_SECRET=$OAUTH_CLIENTS_FLOWS_GATEWAY_CLIENT_SECRET -e ADMIN_OAUTH2_AUTHORIZE=$OAUTH_CLIENTS_FLOWS_GATEWAY_OAUTH2_AUTHORIZE -e ADMIN_OAUTH2_TOKEN=$OAUTH_CLIENTS_FLOWS_GATEWAY_OAUTH2_TOKEN -e ADMIN_OAUTH2_USERINFO=$OAUTH_CLIENTS_FLOWS_GATEWAY_OAUTH2_PROFILE -v /var/local/flows-gateway/flows.json:/home/pm2user/.node-red/flows.json -v /var/local/flows-gateway/ssl:/home/pm2user/ssl -e VAULT_TOKEN -v /var/local/consul/ssl:/etc/vault/ssl local_gw
