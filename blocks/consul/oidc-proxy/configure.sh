...
docker create --net=vlan0 --name consul_oidc -p 9080:80 -v /var/local/letsencrypt:/letsencrypt -v /var/local/consul/ssl:/ssl -v /var/local/consul_openresty/config:/conf -v /var/local/consul_openresty/www:/www canzea/oidc-proxy:latest -c /conf/nginx.conf
