resource "canzea_resource" "cicd-pipeline-deploy" {
    path = "/cicd/config"

    attributes = {
        filename = "ecosystems/${var.es_id}/workspaces/${var.workspace}/pipeline-cci-public.gocd.yaml"
        definition = <<-EOT

            format_version: 3
            pipelines:
                ${var.tenant_id}-cci-public-${var.workspace}:
                    group: ${var.tenant_id}
                    ...
                    materials:
                        charts:
                            ...
                            auto_update: false
                        myupstream:
                            pipeline: ${var.tenant_id}-cci-public
                            stage: publish
                    ...
                    - vault:
                        ...
                        - script: |
                            ...
                            vault read -field data -format yaml secret/tenants/${var.tenant_id}/${var.workspace}/services/cci-public > env.yaml
                    ...
                    - deploy:
                        ...
                        tasks:
                        - fetch:
                            pipeline: ${var.tenant_id}-cci-public-${var.workspace}
                            stage: vault
                            job: vault
                            source: artifacts
                            destination: .
                        ...
                        - script: |
                            ...
                            $PARAMS_FROM_VAULT
                                ...
                                link_base: \"${var.workspace}.ws.${var.domain_name}\"

                            nodeSelector:
                                doks.digitalocean.com/node-pool: ${var.es_id}-${var.workspace}-pool

                            image:
                                repository: registry.ops.${var.domain_name}/${var.tenant_id}/cci-public
                            ...
                            ingress:
                                enabled: true
                                hosts:
                                - www.${var.workspace}.ws.${var.domain_name}
                                - copeconsulting.ca
                                tls:
                                - secretName: cci-ssl
                                  hosts:
                    ...
                    - deploy_static:
                        ...
                        - script: |
                            ...
                            BUCKET_PATH="www.${var.workspace}.ws"
                            ...
                            (cd artifacts/target && s3cmd sync --acl-public . s3://$BUCKET/$TENANT/$BUCKET_PATH/)
        EOT
  }
}
