resource "canzea_resource" "cicd-pipeline-job-manager-app" {
    path = "/cicd/config"

    attributes = {
        filename = "ecosystems/${var.es_id}/workspaces/${var.workspace}/pipeline-canzea-job-manager.gocd.yaml"
        definition = <<-EOT

            format_version: 3
            pipelines:
                ${var.tenant_id}-canzea-job-manager-${var.workspace}:
                    group: ${var.tenant_id}
                    ...
                    - vault:
                        ...
                        - script: |
                            ...
                            vault read -field data -format yaml secret/tenants/${var.tenant_id}/${var.workspace}/services/job-manager > env.yaml
                    ...
                    - deploy:
                        ...
                        tasks:
                        - fetch:
                            pipeline: ${var.tenant_id}-canzea-job-manager-${var.workspace}
                            stage: vault
                            job: vault
                            source: artifacts
                            destination: .
